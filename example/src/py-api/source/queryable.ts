...
// 导出接口BasicQueryParams，用于定义基本查询参数
export interface BasicQueryParams {
  ...
  // 导出接口PagedData，用于定义分页数据
  export interface PagedData<T> { total: number, data: T[] }
  
  
  
  // 导出默认抽象类Queryable，用于查询数据；T为对象类型，E为查询参数类型
  ...
  // 定义一个异步完成器函数，接收一个参数obj
  function asyncCompleter(obj) {
    // 返回一个Promise对象，并传入一个函数，该函数接收一个参数r
  ...
  function defaultObjectBuilder() {
    // 返回一个对象，其中包含id、name、title属性
    return {
      // id属性值为随机数乘以10的整数
      id: Math.floor(Math.random() * 10),
      // name属性值为随机数乘以100000的整数加上字符串"Name#"
      name: "Name#" + Math.floor(Math.random() * 100000),
      // title属性值为随机数乘以100000的整数加上字符串"Title#"
